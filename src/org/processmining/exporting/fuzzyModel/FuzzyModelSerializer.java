...
public class FuzzyModelSerializer {
	...
	public static void serialize(FuzzyGraph model, OutputStream out) throws IOException {
		...
		root.addAttribute("size", Integer.toString(model.getNumberOfInitialNodes()));
		...
		if(model.getAttributeKeys().size() > 0) {
			...
			attributes.addAttribute("size", Integer.toString(model.getAttributeKeys().size()));
		...
		for(int i=0; i<events.size(); i++) {
			...
			logEvent.addAttribute("occurrence", Integer.toString(event.getOccurrenceCount()));
		}
		// add metrics section
		Tag metrics = root.addChildNode("metrics");
		Tag unarySig = metrics.addChildNode("unarySignificance");
		unarySig.addAttribute("size", Integer.toString(model.getNodeSignificanceMetric().size()));
		unarySig.addTextNode(serializeUnaryMetricValues(model.getNodeSignificanceMetric()));
		Tag binarySig = metrics.addChildNode("binarySignificance");
		binarySig.addAttribute("size", Integer.toString(model.getEdgeSignificanceMetric().size()));
		binarySig.addTextNode(serializeBinaryMetricValues(model.getEdgeSignificanceMetric()));
		Tag binaryCorr = metrics.addChildNode("binaryCorrelation");
		binaryCorr.addAttribute("size", Integer.toString(model.getEdgeCorrelationMetric().size()));
		binaryCorr.addTextNode(serializeBinaryMetricValues(model.getEdgeCorrelationMetric()));
		// add transformed metrics section
		Tag transformedMetrics = root.addChildNode("transformedMetrics");
		// transformed significance
		Tag sigTransformed = transformedMetrics.addChildNode("transformedBinarySignificance");
		sigTransformed.addAttribute("size", Integer.toString(model.getNumberOfInitialNodes()));
		...
		Tag corrTransformed = transformedMetrics.addChildNode("transformedBinaryCorrelation");
		corrTransformed.addAttribute("size", Integer.toString(model.getNumberOfInitialNodes()));
		...
		clusters.addAttribute("size", Integer.toString(model.getClusterNodes().size()));
		for(ClusterNode clusterNode : model.getClusterNodes()) {
			Tag cluster = clusters.addChildNode("cluster");
			cluster.addAttribute("index", Integer.toString(clusterNode.getIndex()));
			cluster.addAttribute("name", clusterNode.getElementName());
			Node[] primitives = clusterNode.getPrimitives().toArray(new Node[clusterNode.getPrimitives().size()]);
			builder = new StringBuilder();
