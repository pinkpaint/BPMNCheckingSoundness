...
public class BenchmarkResultBarView extends JComponent {
	...
	public BenchmarkResultBarView(List<BenchmarkMetric> metricList, List<BenchmarkItem> itemList) {
		setBackground(Color.BLACK);
		// invert metric order as will be painted from bottom to top (to preserve displayed order)
		...
		for(int x=items.size()-1; x>=0; x--) {
			for(int y=metrics.size()-1; y>=0; y--) {
				measurements[x][y] = items.get(x).getMeasurement(metrics.get(y).name());
			}
		}
	...
	protected void paintComponent(Graphics g) {
		...
		int yStep = (int)Math.floor(((double)barHeight / (double)((metrics.size() * 2) + 1)));
		...
		for(int i=0; i<items.size(); i++) {
			...
			g2d.drawString(name, xStart + (barWidth / 2) - (stringWidth / 2) , height - vBorder + 20);
			xStart += barOffset;
		}
		// paint metrics names
	...
	protected void paintItemBar(Graphics2D g2d, int index, int x, int y, int width, int height) {
		// prepare data
		int centerX = x + (width / 2);
		int yStep = (int)Math.floor(((double)height / (double)((metrics.size() * 2) + 1)));
		...
		for(int vert=0; vert<metrics.size(); vert++) {
			...
			paintSegment(g2d, centerX, lastY, lastY-yStep, lastOffset, xOffsets[vert], lastColor, yColors[vert]);
			// paint straight segment
			lastY -= yStep;
			paintSegment(g2d, centerX, lastY, lastY-yStep, xOffsets[vert], xOffsets[vert], yColors[vert], yColors[vert]);
			lastY -= yStep;
			lastOffset = xOffsets[vert];
			lastColor = yColors[vert];
		}
		// paint final skewed segment (top closure)
		paintSegment(g2d, centerX, lastY, lastY - yStep, lastOffset, 0, lastColor, BenchmarkResultBarView.TRANSPARENT);
	}
	...
	protected void paintSegment(Graphics2D g2d, int xCenter, int yA, int yB,
		...
		if(colorA.equals(colorB)) {
			...
			GradientPaint gradient = new GradientPaint(xCenter, yA, colorA, xCenter, yB, colorB, false);
			g2d.setPaint(gradient);
		}
		int[] xCoords = {xCenter - offsetA, xCenter - offsetB, xCenter + offsetB, xCenter + offsetA};
