...
/**
 * Precision and Structure metric based on the Minimal Description Length principle.
 * Advocates a balance of the cost of encoding the model and encoding the data with the model.
 *
 * @author Anne Rozinat
 */
public class MinimalDescriptionLengthMetric implements BenchmarkMetric {
	...
	public double measure(PetriNet model, LogReader referenceLog, PetriNet referenceModel, Progress progress) {
		...
		//float compactnessMeasure = structuralResult.getStructuralAppropriatenessMeasure();

		int compactnessMeasure = model.getEdges().size();

		// invoke log replay analysis method --> "Preciseness" measure part
		LogReplayAnalysisMethod logReplayAnalysis = new LogReplayAnalysisMethod(model, referenceLog, new MDLMeasurer(), progress);
		...
		ConformanceLogReplayResult replayResult = (ConformanceLogReplayResult) logReplayAnalysis.analyse(null);
...
//	/**
//	 * Creates the configuration object that is used by the structural analysis to determine
//	 * which data should be collected (in order to calculate metric).
//	 * @return a configuration object that only contains the "saS" fitness metric
//	 */
//	private AnalysisConfiguration createAnalysisConfiguration() {
//		// TODO: think about how to avoid all this repliated configuration (currently easiest solution)
//		// saS
...
//	}
	...
	/**
	 * Class taking all the measurements that are needed to calculate the MDL metric for the given
	 * log and model (cost to encode the log in model) during log replay.
	 * @author Anne Rozinat
	 */
	public class MDLMeasurer extends ConformanceMeasurer {
		...
		protected void takeFailedTaskMeasurement(ReplayedLogTrace pi, ReplayedTransition t, AuditTrailEntry ate) {
			// TODO Remember that current ate has failed execution (will be punished by MDL metric for lack of fitness)
		...
		protected void takePreStepExecutionMeasurement(LogReplayAnalysisResult result, ReplayedLogTrace pi, int maxDepth) {
			...
			DiagnosticPetriNet clonedNet = (DiagnosticPetriNet) result.replayedPetriNet.clone();
		 ...
		 */
		private HashSet<DiagnosticTransition> traceEnabledTasks(HashSet<DiagnosticTransition> alreadyCounted,
				StateSpace coverabilityGraph, State currentState, int maxEnabled, HashSet<State> visitedStates) {
