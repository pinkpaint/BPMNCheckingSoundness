...
public class VariableTranslator {
	 ...
	 * @param g Group the group for which we want to obtain the cpn variable and type
	 ...
	 * @return CpnVarAndType the variable in cpn for the modified data attributes
	 */
	public static CpnVarAndType getCpnVarForModifiedDataAttributes() {
		return new CpnVarAndType(HLToCPNTranslator.cpnVarNameForModifiedDataAttributes, 
				HLToCPNTranslator.cpnColorSetNameForDataAttributes);
	}
	 ...
	 * @param attrib HLAttribute the data attribute for which we want to obtain
	 * a cpn compliant variable
	 * @return CpnVarAndType the variable and type in cpn for the given data attribute
	 */
	 ...
	 * @param probDep the condition for which the probability dependencies are requested
	 * @return CpnVarAndType the cpn variable and type for the probability dependency
	 */
	public CpnVarAndType getCpnVarForProbDep(HLCondition probDep) {
		HLActivity targetAct = probDep.getTarget();
		// when the targetNode is involved in more than one decisionpoint then the
		// cpn var name should also contain the name of the source node. The same for
		// the colorset.
		String nameProbVar = HLToCPNTranslator.cpnVarNameForProbDep;
		if (highLevelPN.getHLProcess().getChoicesForTargetActivity(targetAct.getID()).size() > 1) {
			ModelGraphVertex place = highLevelPN.findModelGraphVertexForChoice(probDep.getChoice().getID());
			nameProbVar = HLToCPNTranslator.cpnVarNameForProbDep + place.getIdentifier();
		}
		CpnVarAndType cpnVar = new CpnVarAndType(nameProbVar, HLToCPNTranslator.cpnColorSetNameForProbDep);
	 ...
	 * @return CpnVarAndType the cpn variable and type for the group that contains
	 * each resource
	 */
	public static CpnVarAndType getCpnVarForGroupAllResources() {
		return new CpnVarAndType(HLToCPNTranslator.cpnVarNameForGroupAllResources, 
				HLToCPNTranslator.cpnColorSetNameForGroupAllResources);
	}

	/**
	 * Returns a list with the cpn vars for all groups, except the cpn-var
	 * that has to represent all resources.
	 * @return ArrayList
	 */
	public ArrayList<CpnVarAndType> getCpnVarsForGroups() {
		ArrayList<CpnVarAndType> returnCpnVars = new ArrayList<CpnVarAndType>();
		Iterator<HLGroup> groups = highLevelPN.getHLProcess().getGroups().listIterator();
