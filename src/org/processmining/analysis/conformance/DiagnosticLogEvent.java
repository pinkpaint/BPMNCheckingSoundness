 ...
 * such as 'A complete' (i.e., including event type and opposed to the audit trail
 * entry, which is an occurrence of a log event).
 *
 * @author anne
 */
public class DiagnosticLogEvent extends LogEvent {
	...
	/**
	 * Contains all log events, which have occurred some time before this log event.
	 */
	private HashMap<DiagnosticLogEvent,
			Integer> preActivities = new HashMap<DiagnosticLogEvent, Integer>();

	/**
	 * Contains all log events, which have occurred some time after this log event.
	 */
	private HashMap<DiagnosticLogEvent,
			Integer> postActivities = new HashMap<DiagnosticLogEvent, Integer>();

	/**
	 * Constructor taking the model element and the event type for this log event.
	 * @param name the name of the task in the corresponding process
	 * @param type the life-cycle stage of this log event with respect to the specified activity
	 */
	 ...
	 * @param event the log event which has been occurred some time ago
	 */
	 ...
	 * indicated value.
	 * @param event the log event which has been occurred some time ago
	 * @param value the number of times this activity newly occurred before this one
	 */
	 ...
	 * @param event the log event which status is requested
	 ...
	 * @param event the log event which status is to be updated
	 */
	...
	/**
	 * Increments the post-occurrence status of the indicated log event by
	 * the indicated value.
	 * @param event the log event which status is to be updated
	 * @param value the number of times this activity newly occurred after this one
	 */
	 ...
	 * @param threshold the number of times that an activity may not have occurred
	 ...
	 * <br>
	 * Note that when calling this method it is assumed that 0 relations are contained
	 * explicitly (rather than being indicated via the absence of that particular pre-
	 * or post-activity).
	 * @param threshold the number of times that an activity may have occurred
	...
	public Set<DiagnosticLogEvent> getSometimesRelationForwards(int threshold) {
		...
		while (it.hasNext()) {
			DiagnosticLogEvent currentPost = it.next();
			if (alwaysRelation.contains(currentPost) == false && neverRelation.contains(currentPost) == false) {
				result.add(currentPost);
			}
		}
		return result;
	 ...
	 * sometimes := all - (always + never)
	 * @return the set of those activities that sometimes preceeded. May be empty
	 */
	public Set<DiagnosticLogEvent> getSometimesRelationsBackwards(int threshold) {
		...
		while (it.hasNext()) {
			DiagnosticLogEvent currentPre = it.next();
			if (alwaysRelation.contains(currentPre) == false && neverRelation.contains(currentPre) == false) {
				result.add(currentPre);
			}
		}
		return result;
