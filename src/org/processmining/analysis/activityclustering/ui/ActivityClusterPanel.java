...
public class ActivityClusterPanel extends JComponent {
	...
	protected void updateTypes() {
		...
		instanceClustersTypes = (List<Integer>[])new List[log.numberOfInstances()];
		...
		for(int i=0; i<log.numberOfInstances(); i++) {
			...
			try {
				AuditTrailEntryList iList = log.getInstance(i).getAuditTrailEntryList();
		...
		for(int i=types.size()-1; i>=0; i--) {
			for(Cluster cluster : types.get(i).getInstances()) {
				...
				try {
					...
					if(instancesStart[instanceIndex] == null || instancesStart[instanceIndex].after(cStart)) {
						instancesStart[instanceIndex] = cStart;
					}
					Date cEnd = cluster.getEnd();
					if(instancesEnd[instanceIndex] == null || instancesEnd[instanceIndex].before(cEnd)) {
	...
	protected void paintComponent(Graphics g) {
		...
		/*
		long lLeft = lStart + (long)((double)duration * pctLeft);
		long lRight = lStart + (long)((double)duration * pctRight);
		Date dLeft = new Date(lLeft);
		Date dRight = new Date(lRight); */
			...
			if(y <= clipLow && yLow >= clip.y) {
				...
				for(int j=0; j<instanceClusters[i].size(); j++) {
					try {
						...
						Date cStart = cluster.getStart();
						Date cEnd = cluster.getEnd();
						if(true) {//cEnd.getTime() >= liLeft && cStart.getTime() <= liRight) {
							// cluster intersects viewport, draw!
							int xStart = translateToXCoordinateInstance(i, cStart.getTime());
							int xEnd = translateToXCoordinateInstance(i, cEnd.getTime());
							if(xStart == xEnd) { 
								if(cStart.equals(instancesStart[i]) && cEnd.equals(instancesEnd[i])) {
									xStart = 0;
									xEnd = width - 1;
								} else {
									xEnd = xStart + 2;
							...
							//System.out.println(xStart + " -> " + xEnd + " -- " + cStart + " -> " +cEnd);
							int yStart = y + 3 + instanceClustersTypes[i].get(j) % this.maxOffset;
	...
	protected int translateToXCoordinateInstance(int instance, long timestamp) {
		long normalizedPosition = timestamp - instancesStart[instance].getTime();
		long duration = instancesEnd[instance].getTime() - instancesStart[instance].getTime();
