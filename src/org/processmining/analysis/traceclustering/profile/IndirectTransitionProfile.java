...
public class IndirectTransitionProfile extends AbstractProfile {
	...
	public IndirectTransitionProfile(LogReader log) 
		...
		super("Indirect Transition", "Compares event pairs (a,b) such that b eventually follows a", log);
		buildProfile(log);
	}
	...
	protected static String encodeTransition(AuditTrailEntry ateA, AuditTrailEntry ateB) {
		return ateA.getElement() + "--" 
			+ ateA.getType() + "-->" 
			+ ateB.getElement() + "--" 
			+ ateB.getType();
	}
