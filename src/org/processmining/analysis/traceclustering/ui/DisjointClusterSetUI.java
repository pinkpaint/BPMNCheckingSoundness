 ...
 *
 */
public class DisjointClusterSetUI extends JComponent implements MouseListener, MouseMotionListener {
	...
	protected void paintComponent(Graphics g) {
		...
		if(recomRasterSize != this.rasterSize || this.buffer == null) {
			...
			buffer = new BufferedImage(bufferSize, bufferSize, BufferedImage.TYPE_INT_RGB);
			...
			for(int traceX=0; traceX<numberOfTraces; traceX++) {
				for(int traceY=0; traceY<numberOfTraces; traceY++) {
					...
					double colorVal = 1.0 - distanceMatrix.get(traceMappedX, traceMappedY);
					Color color = ColorRepository.getGradualColorBlackZero(colorVal);
					gBuf.setColor(color);
					if(recomRasterSize == 1) {
						gBuf.drawLine(traceMappedX, traceMappedY, traceMappedX, traceMappedY);
					} else {
		...
		for(int i=0; i<clusterSizes.size(); i++) {
			...
			// set color
			if((i % 2) == 0) {
				if(mouseOver && mouseX > lastSize && mouseX <= currentOuterCoord) {
					g2d.setColor(colorEvenMouseOver);
				} else {
					g2d.setColor(colorEven);
				}
				g2d.fillRect(lastSize, fixC, currentSize, fixA - fixC);
				if(mouseOver && mouseY > lastSize && mouseY <= currentOuterCoord) {
					g2d.setColor(colorEvenMouseOver);
				} else {
					g2d.setColor(colorEven);
				}
		...
		if(mouseOver == true) {
			...
			if(mouseX < sumSize || mouseY < sumSize || mouseX > boundary || mouseY > boundary) {
				// invalid, ignore
			} else {
				// find selected cluster combination
				for(int i=0; i<clusterSizes.size(); i++) {
					int outerBound = sumSize + (clusterSizes.get(i) * this.rasterSize);
				...
				drawInfo(g2d, width, height, selectX, selectY, startX, startY, endX, endY);
			}
		}
		// finished drawing
		g2d.dispose();
	...
	protected void drawInfo(Graphics2D g2d, int width, int height, int selectX, int selectY, int startX, int startY, int endX, int endY) {
		final int border = 10;
		...
		if(selectX == selectY) {
			...
			lines.add("  " + clusterX.getName() + " (contains " + clusterX.size() + " traces)");
			lines.add("  " + clusterY.getName() + " (contains " + clusterY.size() + " traces)");
		}
		// compute size and position of info overlay
		int textWidth = 0;
		for(String line : lines) {
			textWidth = Math.max(textWidth, (int)fontMetrics.getStringBounds(line, g2d).getWidth());
		}
