...
public abstract class HmmNoiseEvaluator {
	...
	protected Map<Double,LogReader> noisyLogs; // noise level and the corresponding log
	protected HashMap<Double,ArrayList<LogReader>> replicatedNoisyLogs; // noise level and list of replicated logs
	protected ArrayList<HmmExpMetric> metrics;
	
	protected String typeNoiseFolder; // where to read the noisy logs from
	protected String typeEvalFolder; // where to put the results of the evaluation
	 ...
	 * @param model
	 */
	public HmmNoiseEvaluator(Map<String,PetriNet> models, HmmExpConfiguration aConf) {
		inputModels = models;
		conf = aConf;
	...
	/**
	 * Evaluates all logs in the 'NoisyLogs' directory with respect to the 
	 * given Petri net model and outputs the result in a file 'NoiseEvaluation.gpl'.
	 */
	...
	private void writeGnuplotScripts() throws IOException {
		// values with lines
		GnuplotScript gnuplot = new GnuplotScript(typeEvalFolder, "FitnessEvaluation", 
		...
		gnuplot = new GnuplotScript(typeEvalFolder, "FitnessRatioEvaluation", 
				"Fitness Ratios for Different Noise Levels",
				"Noise Level", "Fitness Value", PlotType.lines);
		for (HmmExpMetric metric : metrics) {
	...
	private void writeReplicateGnuplotScripts() throws IOException {
		// values with lines
		GnuplotScript gnuplot = new GnuplotScript(typeEvalFolder, "FitnessReplicationEvaluation", 
		...
		gnuplot.finish();
		// values with points
		gnuplot = new GnuplotScript(typeEvalFolder, "FitnessReplicationEvaluation", 
				"Average Fitness Measurements for Different Noise Levels",
		...
		gnuplot.finish();
		// ratios with lines
		gnuplot = new GnuplotScript(typeEvalFolder, "FitnessRatioReplicationEvaluation", 
				"Average Fitness Ratios for Different Noise Levels",
				"Noise Level", "Fitness Value", PlotType.lines);
		for (HmmExpMetric metric : metrics) {
			gnuplot.addPlot(metric.getFilename() + "AverageRatio", metric.getName());
		}
		gnuplot.finish();
		// individual ratios
		for (HmmExpMetric metric : metrics) {
	...
	private void evaluateNoisyLogs(PetriNet inputModel) throws IOException {
		...
		for (int i=0; i<noiseLevels.length; i++) {
			...
			for (HmmExpMetric metric : metrics) {
				double value = metric.calculateValue(analyzer, inputModel, currentLog);
				writeToDat(metric.getValueWriter(), noiseLevels[i].toString(), value);
				writeToDat(metric.getRatioWriter(), noiseLevels[i].toString(), calculateRatio(((Double)noiseLevels[i]).doubleValue(), value));
			}
		}
		for (HmmExpMetric metric : metrics) {
	...
	private void evaluateReplicatedNoisyLogs(PetriNet inputModel) throws IOException {
		BufferedWriter globalMeasures = HmmExpUtils.createWriter(typeEvalFolder, "Global", "txt");
		...
		for (int i=0; i<noiseLevels.length; i++) {
			...
			for (LogReader log : replicatedNoisyLogs.get(noiseLevels[i])) {
				...
				for (HmmExpMetric metric : metrics) {
					...
					double val = metric.calculateValue(analyzer, inputModel, log);
					metValues.addValue(val);
					SummaryStatistics ratioValues = globalRatioStatistics.get(metric);
					ratioValues.addValue(calculateRatio(((Double)noiseLevels[i]).doubleValue(), val));
				}
			}
			for (HmmExpMetric metric : metrics) {
				SummaryStatistics metValues = values.get(metric);
				System.out.println("Average " + metric.getName() + ": " + metValues.getMean());
				writeToDat(metric.getAverageWriter(), noiseLevels[i].toString(), metValues.getMean());
				writeToDat(metric.getVarianceWriter(), noiseLevels[i].toString(), metValues.getVariance());
				globalAverageVariance.get(metric).addValue(metValues.getVariance());
				double ratio = calculateRatio(((Double)noiseLevels[i]).doubleValue(), metValues.getMean());
				writeToDat(metric.getAverageRatioWriter(), noiseLevels[i].toString(), ratio);
		...
		for (HmmExpMetric metric : metrics) {
			...
			SummaryStatistics ratioValues = globalRatioStatistics.get(metric);
			globalMeasures.write("\nGlobal Average Ratio " + metric.getName() + ": " + avRatio.getMean() + "\n");
			globalMeasures.write("Global Average Variance " + metric.getName() + ": " + avVariance.getMean() + "\n");
			globalMeasures.write("Total Mean ratio " + metric.getName() + ": " + ratioValues.getMean() + "\n");
			globalMeasures.write("Total Variance ratio " + metric.getName() + ": " + ratioValues.getVariance() + "\n");
			metric.finishDatFiles();
		}
	...
	/**
	 * Calculates the noise-fitness ratio, assuming that they should be 
	 * inverse proportional to each other (the bigger the degree of noise, 
	 * the lower the fitness). 
	 * <br>
	 * Return value 0.0 is ideal. 
	 * Positive value indicates optimistic fitness measurement.
	 * Negative value indicates pessimistic fitness measurement. 
	 * @param noise the current noise level in per cent
	 * @param fitness the fitness value in [0, 1]
	 ...
	 * @param writer the file to which it should be written
	 * @param explevel the current level of noise or completeness
	 * @param value the value to be added
	 * @throws IOException
	 */
	private void writeToDat(BufferedWriter writer, String explevel, double value) throws IOException {
	...
	private void getLogs(File file, Double noiseLevel) throws Exception {
		if (conf.isReplicate() == false) { // read logs directly form noisy log directory
	...
	private LogReader readlLogFromFile(File file) throws Exception {
		...
		LogReader log = BufferedLogReader.createInstance(new DefaultLogFilter(DefaultLogFilter.INCLUDE), logFile);
		return log;
	}
