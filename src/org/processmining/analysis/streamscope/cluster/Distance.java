...
public class Distance {
	...
	public static Distance create(LogReader log, Progress progress) throws IOException {
		...
		for(int i=0; i<log.numberOfInstances(); i++) {
			...
			for(int k=1; k<ateList.size(); k++) {
				...
				for(int l=(k-1); l>(k-MAX_LOOK_BACK) && l>=0; l--) {
					AuditTrailEntry back = ateList.get(l);
					int backIndex = ecTable.getIndex(back.getElement());
					double distance = staticDistance(back, current) * attenuationFactor;
				...
				*/
				for(int m=0; m<lookBackIndices.size(); m++) {
					...
					double distance = staticDistance(backEntry, current) * attenuationFactor;
					distances[currentIndex][backIndex] += distance;
					distances[backIndex][currentIndex] += distance;
					frequencies[currentIndex][backIndex] += attenuationFactor;
					frequencies[backIndex][currentIndex] += attenuationFactor;
		...
		/*
		// remove frequency factor
		for(int x=0; x<ecTable.size(); x++) {
			for(int y=0; y<ecTable.size(); y++) {
				distances[x][y] /= frequencies[x][y];
			}
		}
		*/
		// normalization
		/*
		for(int i=0; i<ecTable.size(); i++) {
			for(int k=0; k<ecTable.size(); k++) {
				distances[i][k] /= maxDistance;
			}
		}
		*/
		// here we go!
	...
	public static double staticDistance(AuditTrailEntry ateA, AuditTrailEntry ateB) {
		return 1.0;
	}
