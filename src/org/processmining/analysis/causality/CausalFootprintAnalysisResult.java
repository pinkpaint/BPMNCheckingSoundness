...
/*findDeadlock Pattern*/
	private ArrayList findDeadlockPatterns() {
		...
		while (it.hasNext()) {
			...
			// B = edge.destinations
			//Fla = (a,B); Flb = (A,b)
			HashSet outgoing = cs.getOutgoingLookAheadEdges(edge.source); //lấy hết các cạnh ra từ đỉnh nguồn
			...
			while (it2.hasNext()) {
				...
				if ((e.destinations.size() == 1) &&
					...
					if (allFromB) {
						System.out.println("Found a Deadlock Of Edge: " + edge.source.toString() + " - "+ edge.destinations.toString());
						
						System.out.println("Forward edge e: " + e.source.toString()+" - "+e.destinations.toString());
						// Deadlock found
						HashSet result = new HashSet();
						result.add(edge.source);
						System.out.println("Add to Deadlock pattern source of edge: " + edge.source.toString());
						result.addAll(edge.destinations);
						System.out.println("Add to Deadlock pattern destinations of edge: " + edge.destinations);
						result.addAll(e.destinations);
						System.out.println("Add to Deadlock pattern destinations of e: " + e.destinations);
						pat.add(result);
					}
				}
			}
