...
public class OptionPanel implements GuiDisplayable {
	...
	public enum ResultType {
		...
		SUM("Sum"),
		NO_MEASUREMENTS("No. of Measurements");
	...
	public OptionPanel() {
		...
		myViewingProperties = new GUIPropertyListEnumerationOfGuiDisplayables(
				"Display results as:", "Determine in which mode the results shall be displayed", viewingOptions, null, 200);
		...
		myShowArithmeticMeanValues = new ResultConfiguration("Arithmetic Mean", true, timeOptions); 
		myShowVarianceValues = new ResultConfiguration("Variance", false); 
		myShowStandardDeviationValues = new ResultConfiguration("Standard Deviation", true);
		myShowGeometricMeanValues = new ResultConfiguration("Geometric Mean", true); 
		myShowSumValues = new ResultConfiguration("Sum", true, timeOptions); 
		myShowNoMeasurementsValues = new ResultConfiguration("No. of measurements", true);	
		...
		myDisplayProperties = new GUIPropertyListEnumerationOfGuiDisplayables(
				"Adjust properties: ", "Change the properties for specific result values", allProperties, null, 200);
	}
	
	/**
	 * Returns the currently selected view mode object, which contains
	 * further information about the display options for the respective 
	 * mode.
	 ...
	 * The configuration object contains information about, e.g., in which 
	 * time unit the result type should be displayed.
	 * @param type the result type for which the configuration object is requested
	...
	/**
	 * Configures a specific type of basic log statistic result (e.g., minimum value).
	 * <br>
	 * It captures whether this option should be included in the HTML view, and optionally in which 
	 * time unit it shall be displayed.
	 * 
	 * @author anne
	 */
	public class ResultConfiguration implements GuiDisplayable {
		...
		public ResultConfiguration(String name, boolean isEnabled, ArrayList timeValues) {
			myName = name;
	...
	class TextViewProperties extends ViewMode implements GuiDisplayable {
		...
		public TextViewProperties() {
			...
			mySortByProperty = new GUIPropertyListEnumeration(
					"Sort by:  ", "Determine by what the list the results should be sorted", values, null, 180);
		}
		
		/**
		 * Retrieves the result type by which the HTML table should be 
		 * sorted.
		 * @return the result type by which the activity durations should be sorted
		 */
	...
	public class GraphViewProperties extends ViewMode implements GuiDisplayable {
		...
		public GraphViewProperties() {
			...
			myVisualizeProperty = new GUIPropertyListEnumeration(
					"Visualize:", "Determine which kind of values should be visualized", values, null, 180);
		}
		
		/**
		 * Retrieves the result type currently selected for visualization.
		 * @return the result type which shall be visualized for the activity durations
		 */
		public ResultType getValue() {
