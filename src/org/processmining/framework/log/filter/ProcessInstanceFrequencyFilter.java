...
/**
 * Log filter that keeps all processs instances with a No. of grouped instances above the given 
 * frequency threshold. 
 * 
 * @author Anne Rozinat
 */
public class ProcessInstanceFrequencyFilter extends LogFilter {
	...
	protected boolean doFiltering(ProcessInstance instance) {
		return (MethodsForWorkflowLogDataStructures.getNumberSimilarProcessInstances(instance) > lowerLimitFrequency);
	}
	...
	public LogFilterParameterDialog getParameterDialog(LogSummary summary) {
		...
		return new LogFilterParameterDialog(summary, ProcessInstanceFrequencyFilter.this) {
			...
			protected JPanel getPanel() {
				...
				piFreq = new GUIPropertyIntegerTextField("Frequency Threshold", 0, 0, Integer.MAX_VALUE);
