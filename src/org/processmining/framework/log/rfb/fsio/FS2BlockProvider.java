...
public class FS2BlockProvider {
	...
	protected synchronized static MappedByteBuffer requestMap(FS2BlockProvider requester) 
			throws IOException {
		synchronized(FS2BlockProvider.class) {
			...
			if(currentShadowSize < shadowSize) {
				...
				currentShadowSize++;
				Message.add("NikeFS2: Populating shadow map " + currentShadowSize + " (of " + shadowSize + " max.)", Message.DEBUG);
				return map;
			} else {
				...
				Message.add("NikeFS2: Displacing shadow map " + kickIndex, Message.DEBUG);
				System.gc();
				return map;
			}
		}
	...
	public FS2BlockProvider(File storage, int size, int blockSize, boolean mapped) 
			throws IOException {
		synchronized(this) {
			this.mapped = mapped;
			this.size = size;
