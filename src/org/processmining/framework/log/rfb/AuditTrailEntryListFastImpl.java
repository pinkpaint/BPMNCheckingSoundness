...
public class AuditTrailEntryListFastImpl implements AuditTrailEntryList {
	...
	public AuditTrailEntryListFastImpl(ATERandomFileBuffer buffer, int maxFragment) throws IOException {
		this.size = 0;
		this.buffer = buffer;
		this.holeFlags = new BitSet();
		this.overflowIndices = new int[OVERFLOW_LIMIT];
	...
	public synchronized boolean consolidate() throws IOException {
		if (isTainted()) {
			...
			ATERandomFileBuffer nBuffer = new ATERandomFileBuffer(LogReaderMenu.storageProvider);
			int overflowIndex = 0;
			int fileBufferIndex = 0;
			for (int i = 0; i < size; i++) {
				if (overflowIndex < overflowSize && overflowIndices[overflowIndex] == i) {
	...
	public synchronized AuditTrailEntry get(int index) throws IndexOutOfBoundsException, IOException {
		...
		// respectively
		for (int hole = holeFlags.nextSetBit(0); hole >= 0 && hole <= bufferIndex; hole = holeFlags
				.nextSetBit(hole + 1)) {
			bufferIndex++;
		}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.processmining.framework.log.AuditTrailEntryList#insert(org.processmining.framework.log.AuditTrailEntry,
	 *      int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.processmining.framework.log.AuditTrailEntryList#replace(org.processmining.framework.log.AuditTrailEntry,
	 *      int)
	 */
	public synchronized AuditTrailEntry replace(AuditTrailEntry ate, int index) throws IndexOutOfBoundsException,
			IOException {
		// check overflow list and adjust indices
