...
public class RecommendationResultMarshal {
	...
	public String marshal(RecommendationResult result) throws Exception {
		...
		root.addAttribute("partialTraceValue", Double.toString(result.getPartialTraceValue()));
...
//			recNode.addAttribute("confidence", Double.toString(rec.getConfidence()));
			recNode.addAttribute("doExpectedValue", Double.toString(rec.getDoExpectedValue()));
			recNode.addAttribute("dontExpectedValue", Double.toString(rec.getDontExpectedValue()));
			recNode.addAttribute("doExpectedSquaredValue", Double.toString(rec.getDoExpectedSquaredValue()));
			recNode.addAttribute("dontExpectedSquaredValue", Double.toString(rec.getDontExpectedSquaredValue()));
			recNode.addAttribute("doWeight", Double.toString(rec.getDoWeight()));
			recNode.addAttribute("dontWeight", Double.toString(rec.getDontWeight()));
...
//				currentRecommendation.setWeight(weight);
		...
		public void endElement(String uri, String localName, String qName) throws SAXException {
			...
			if (tagName.equalsIgnoreCase("Recommendation")) {
				...
				currentRecommendation = null;
			} else if (tagName.equalsIgnoreCase("Task") && currentRecommendation != null) {
				// set task
				currentRecommendation.setTask(buffer.toString().trim());
			} else if (tagName.equalsIgnoreCase("User") && currentRecommendation != null) {
				// set user
				currentRecommendation.addUser(buffer.toString().trim());
			} else if (tagName.equalsIgnoreCase("Role") && currentRecommendation != null) {
				// set user
				currentRecommendation.addRole(buffer.toString().trim());
			} else if (tagName.equalsIgnoreCase("Group") && currentRecommendation != null) {
				// set user
				currentRecommendation.addGroup(buffer.toString().trim());
			} else if (tagName.equalsIgnoreCase("EventType") && currentRecommendation != null) {
				// set event type
				currentRecommendation.setEventType(buffer.toString().trim());
			} else if (tagName.equalsIgnoreCase("rationale") && currentRecommendation != null) {
				// set rationale
		...
		public void error(SAXParseException arg0) throws SAXException {
			System.err.println("Error parsing recommendation result (unmarshalling):");
			arg0.printStackTrace();
		}

		public void fatalError(SAXParseException arg0) throws SAXException {
			System.err.println("Fatal error parsing recommendation result (unmarshalling):");
			arg0.printStackTrace();
			result = null;
		}
