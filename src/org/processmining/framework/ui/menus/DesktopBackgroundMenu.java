...
public class DesktopBackgroundMenu extends JMenu implements ActionListener {
	...
	public DesktopBackgroundMenu() {
		...
		String selectedBgPath = UISettings.getInstance().getPreferredDesktopBackground();
		File selectedBg = new File(selectedBgPath);
		backgrounds = new File[0];
		// collect background directory candidates
		File bgDir = new File(UISettings.getProMDirectoryPath() + "images/desktop/");
		try {
		...
		if(bgDir.exists() == true) {
			...
			for(int i=0; i<protoBg.length; i++) {
				if(protoBg[i].isFile() &&
					...
					Message.add("Desktop background found at " + protoBg[i].getAbsolutePath(), Message.DEBUG);
				} else {
					Message.add("Skipping file " + protoBg[i].getAbsolutePath(), Message.DEBUG);
				}
			}
			backgrounds = bgList.toArray(backgrounds);
		} else {
			Message.add("Desktop background directory not found! (" + bgDir.getAbsolutePath() + ")", Message.ERROR);
		}
		// build menu
		...
		if(command.equals("NONE")) {
			...
			for(int i=0; i<backgrounds.length; i++) {
				if(command.equals(backgrounds[i].getAbsolutePath())) {
					UISettings.getInstance().setPreferredDesktopBackground(command);
