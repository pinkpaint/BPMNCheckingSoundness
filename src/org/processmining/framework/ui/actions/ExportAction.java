...
public class ExportAction extends AbstractAction {
	...
	public ExportAction(ExportPlugin algorithm, ProvidedObject object) {
		super(RuntimeUtils.stripHtmlForOsx("<html>" /*+ object.getName() + " to<br>&nbsp;&nbsp;&nbsp;" */ +
				algorithm.getName() +
				"</html>"));
	...
	public void actionPerformed(ActionEvent e) {
		final String filename = Utils.saveFileDialog(MainUI.getInstance().getDesktop(),
								new GenericFileFilter(algorithm.getFileExtension()));
			...
			SwingWorker closer = new SwingWorker() {
					...
					try {
						...
						try {
							if (algorithm instanceof DoNotCreateNewInstance) {
								...
								((ExportPlugin) algorithm.getClass().newInstance()).export(object, out);
							}
				...
				public void finished() {
					if (get() != null && get() instanceof Exception) {
						Message.add(filename + ": " + ((Exception) get()).getMessage());
						MainUI.getInstance().addAction(algorithm, LogStateMachine.ATE_ABORT,
