...
public class ImportAnyFileAction extends CatchOutOfMemoryAction {
	...
	public ImportAnyFileAction(MDIDesktopPane desktop, boolean useJFileChooser) {
		...
		putValue(SHORT_DESCRIPTION, "Open any type of file supported by ProM  " + getShortcut());
	...
	protected void executeWithAWTFileChooser() {
		FileDialog dialog = new FileDialog(MainUI.getInstance(), "Open file...", FileDialog.LOAD);
		dialog.setFilenameFilter(new ImportAnyFilenameFilter());
		dialog.setDirectory((new File(UISettings.getInstance().getLastOpenedImportFile())).getParent());
		dialog.setFile((new File(UISettings.getInstance().getLastOpenedImportFile())).getName());
	...
	protected void executeWithJFileChooser() {
		JFileChooser dialog = new JFileChooser(new File(UISettings.getInstance().
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			for (File file : dialog.getSelectedFiles()) {
				UISettings.getInstance().setLastOpenedImportFile(file.getAbsolutePath());
				...
				if (plugin instanceof MxmlImportPlugin) {
					...
					if (file.getName().toLowerCase().endsWith(".zip")) {
						OpenMXMLLogDialog logDialog = new OpenMXMLLogDialog(file.getAbsolutePath());
					...
					UISettings.getInstance().addRecentFile(file.getAbsolutePath(), plugin.getName());
					MainUI.getInstance().importFromFile(plugin, file.getAbsolutePath(), connectLog);
				}
			}
		}
	}
	...
	protected LogReader getLogToConnect() {
				...
				for(ProvidedObject providedObject : providedObjects) {
					for(Object object : providedObject.getObjects()) {
						if(object instanceof LogReader) {
							logReaders.put("Open with log: " +
									frame.getTitle() + " - " + providedObject.getName(),
									(LogReader)object);
						}
					}
				}
		...
		String picked = (String) JOptionPane.showInputDialog(
				MainUI.getInstance(),
				"The file you have chosen can optionally be\n" +
	...
	protected List<ImportPlugin> getCompatiblePlugins(File file) {
		...
		ImportPluginCollection importPlugins = ImportPluginCollection.getInstance();
