...
public class FuzzyEdgeEditor extends JPanel {
	...
	public FuzzyEdgeEditor(FuzzyGraphEditorPanel parent, GGEdge edge, ActionListener closeListener) {
		this.parent = parent;
		this.graphEdge = edge;
		this.fuzzyEdge = (Edge)edge.getAttribute(GGAdapter.ATTR_FUZZY_GRAPH_ELEMENT);
		this.graph = (MutableFuzzyGraph)edge.getAttribute(GGAdapter.ATTR_FUZZY_GRAPH);
	...
	protected void initializeGui() {
		...
		// significance slider
		significanceSlider = new NiceDoubleSlider("significance", 0, 1.0, fuzzyEdge.getSignificance(), Orientation.VERTICAL);
		if(isConnectedToCluster) {
			...
			significanceSlider.addChangeListener(new ChangeListener() {
				public void stateChanged(ChangeEvent e) {
					...
					graph.setBinarySignificance(sourceIndex, targetIndex, significanceSlider.getValue());
					graph.getEdgeSignificanceMetric().setMeasure(sourceIndex, targetIndex, significanceSlider.getValue());
		...
		// correlation slider
		correlationSlider = new NiceDoubleSlider("correlation", 0, 1.0, fuzzyEdge.getCorrelation(), Orientation.VERTICAL);
		if(isConnectedToCluster) {
			...
			correlationSlider.addChangeListener(new ChangeListener() {
				public void stateChanged(ChangeEvent e) {
					...
					int sourceIndex = fuzzyEdge.getSource().getIndex();
					int targetIndex = fuzzyEdge.getTarget().getIndex();
					graph.setBinaryCorrelation(sourceIndex, targetIndex, correlationSlider.getValue());
					graph.getEdgeCorrelationMetric().setMeasure(sourceIndex, targetIndex, correlationSlider.getValue());
		...
		deleteEdgeButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				SwingUtilities.invokeLater(new Runnable() {
					public void run() {
						closeListener.actionPerformed(new ActionEvent(this, 0, "edge deleted"));
					}
				});
			}
		});
		...
		this.add(Box.createVerticalStrut(15));
		this.add(FuzzyGraphEditorPanel.centerHorizontally(deleteEdgeButton, 25));
