 ...
 *
 */
public class TokenLabelPainter extends Painter {

	/**
	 * @param canvas
	 */
	public TokenLabelPainter(AnimationCanvas canvas) {
		super(canvas);
	...
	public void paint(long modelTime, Graphics2D g2d) {
		g2d.setColor(canvas.getColorSettings().getTokenLabel());
		float offset = canvas.translateY(canvas.getBoxHeight()) * 0.4f;
		float fontSize = (canvas.getFontSize() * 1.2f);
		...
		for(ArcAnimation anim : arcAnimations) {
			...
			for(TokenAnimation tokenAnim : keyframe.getTokenAnimations()) {
				...
				float x = canvas.translateX(pos[0]) + offset;
				float y = canvas.translateY(pos[1]) + (fontSize / 2f);
				g2d.drawString(tokenAnim.getCaseId(), x, y);
			}
		}
	}
