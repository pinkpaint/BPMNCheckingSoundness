...
public class TokenRingsPainter extends Painter {
	...
	public void paint(long modelTime, Graphics2D g2d) {
		...
		Color color = ColorSettings.setAlpha(canvas.getColorSettings().getTokenRing(), 0.3f * (1f - percentage));
		g2d.setColor(color);
		BasicStroke stroke = new BasicStroke((1f - percentage) * 0.2f * boxHeight, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND);
		...
		for(ArcAnimation anim : arcAnimations) {
			...
			for(TokenAnimation tokenAnim : keyframe.getTokenAnimations()) {
				...
				g2d.draw(new Ellipse2D.Float(x - radius, y - radius, diameter, diameter));
			}
		}
	}
