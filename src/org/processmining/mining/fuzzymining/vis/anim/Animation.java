...
public class Animation {
	...
	public static Animation generate(FuzzyGraph graph, LogReader log, 
		...
		Animation animation = new Animation(graph, log, maxLookahead, maxExtraLookahead);
	...
	public Animation(FuzzyGraph graph, LogReader log, int maxLookahead, int maxExtraLookahead) {
		this.log = log;
		this.graph = graph;
	...
	public void initialize(ProgressPanel progress) throws IndexOutOfBoundsException, IOException {
		...
		for(Node node : graphNodes) {
			...
			TaskAnimation anim = new TaskAnimation(node, id, type, 
		...
		for(int i=0; i<log.numberOfInstances(); i++) {
			progress.setNote("Reading log: trace " + (i + 1 ) + " / " + log.numberOfInstances());
	...
	protected void parse(ProcessInstance instance, ProgressPanel progress) throws IndexOutOfBoundsException, IOException {
		int counter = 0;
		String caseId = instance.getName();
		AuditTrailEntryList ateList = instance.getAuditTrailEntryList();
		// insert case animation
		TokenAnimation caseTokenAnim = new TokenAnimation(caseId, ateList.get(0).getTimestamp().getTime(),
				ateList.get(ateList.size() - 1).getTimestamp().getTime());
		...
		for(int i=0; i<ateList.size(); i++) {
			...
			for(int j=i+1; (j<=i+maxLookahead) && (j<ateList.size()); j++) {
				...
				if(edge != null) {
					...
					TokenAnimation tokenAnim = new TokenAnimation(caseId, 
							ref.getTimestamp().getTime(), comp.getTimestamp().getTime());
					ArcAnimation arcAnim = arcAnimations.get(createEdgeId(refNode, compNode));
			...
			TaskAnimationKeyframe taskKeyframe = new TaskAnimationKeyframe(caseId, 
					ref.getTimestamp().getTime(), !seeded[i], swallowed);
