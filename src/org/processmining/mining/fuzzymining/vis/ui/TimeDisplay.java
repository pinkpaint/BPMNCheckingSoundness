...
public class TimeDisplay extends JComponent implements AnimationListener {
	...
	protected static final String[] DAY =
	...
	protected void paintComponent(Graphics g) {
		...
		RoundRectangle2D.Float clip = new RoundRectangle2D.Float(0f, 0f, (float)width, (float)height, 2 * border, 2 * border);
		...
		if(this.calendar != null) {
			...
			secondA = second / 10;
			secondB = second % 10;
			day = DAY[this.calendar.get(Calendar.DAY_OF_WEEK) - 1] + " " + this.calendar.get(Calendar.DAY_OF_MONTH);
		...
		gradient = new GradientPaint(0, 0, new Color(200, 200, 200, 10), 
				0, border + (cipherHeight / 2), new Color(200, 200, 200, 40), false);
		g2d.setPaint(gradient);
		g2d.fillRect(0, 0, width + 1, border + (cipherHeight / 2));
		gradient = new GradientPaint(0, cipherHeight / 2 + border, new Color(200, 200, 200, 40), 
				0, cipherHeight / 2 + border + 3, new Color(200, 200, 200, 0), false);
		g2d.setPaint(gradient);
	...
	protected void paintCipher(int n, Graphics2D g2d, int x, int y) {
		...
		g2d.fillPolygon(translate(LEFTBOTTOM_X, x), translate(LEFTBOTTOM_Y, y), 4);
		...
		g2d.fillPolygon(translate(RIGHTBOTTOM_X, x), translate(RIGHTBOTTOM_Y, y), 4);
