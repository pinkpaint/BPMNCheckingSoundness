...
public class AggregateUnaryMetric extends UnaryMetric {
	 ...
	 * @param aDescription
	 */
	public AggregateUnaryMetric(String name, String description, List<UnaryMetric> componentMetrics) {
		super(name, description, componentMetrics.get(0).size);
		metrics = componentMetrics;
	}

	/* (non-Javadoc)
	 * @see org.processmining.mining.fuzzymining.metrics.UnaryMetric#getMeasure(int)
	 */
	public double getMeasure(int index) {
		if(normalized == null) {
			// lazy aggregation and normalization
			ArrayList<UnaryMetric> validMetrics = new ArrayList<UnaryMetric>(metrics.size());
