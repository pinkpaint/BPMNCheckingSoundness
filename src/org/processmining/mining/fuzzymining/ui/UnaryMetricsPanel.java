 ...
 *
 */
public class UnaryMetricsPanel extends JComponent implements MouseListener, MouseMotionListener {
	...
	protected void paintComponent(Graphics g) {
		...
		for(int i=0; i<colors.length; i++) {
			colors[i] = ColorRepository.getGradualColor(((double)(i)) / ((double)colors.length - 1));
		}
		// prepare drawing
		...
		if(width > measurePoints) {
			...
			for(int i=0; i<metrics.size(); i++) {
				...
				int x = 0;
				int lastY = 5 + chartHeight - (int)(metric.getMeasure(0) * (double)chartHeight);
				for(int k=1; k<measurePoints; k++) {
					int y = 5 + chartHeight - (int)(metric.getMeasure(k) * (double)chartHeight);
			...
			for(int i=0; i<metrics.size(); i++) {
				...
				int lastY = 5 + chartHeight - (int)(aggregateValues(0, clusterSize, metric) * (double)chartHeight);
				for(int x=1; x<actualWidth; x++) {
					int y = 5 + chartHeight - (int)(aggregateValues(x, clusterSize, metric) * (double)chartHeight);
	...
	protected void drawInfo(Graphics2D g2d, int chartX, int eventIndex, int chartHeight, Color[] colors) {
		if(mouseX < 0) {
		...
		FontMetrics fontMetrics = this.getFontMetrics(g2d.getFont().deriveFont(12.0f));
	...
	protected double aggregateValues(int step, int clusterSize, UnaryMetric metric) {
		double value = 0.0;
