 ...
 *
 */
public class FastTransformerPanel extends JPanel
		implements ChangeListener, ItemListener, Provider {
	...
	public FastTransformerPanel(MetricsRepository metrics, int showNumberOfNodes) {
		this(new MutableFuzzyGraph(metrics), metrics.getLogReader(), showNumberOfNodes);
	}
	...
	public FastTransformerPanel(MutableFuzzyGraph graph, LogReader log, int showNumberOfNodes) {
		this.log = log;
		this.graph = graph;
	...
	protected void initializeGui() {
		...
		nodeSignificanceSlider.setToolTipText(
				"<html>The lower this value, the more<br>" +
		...
		edgesBestRadioButton.setToolTipText(
				"<html>Activates the 'Best edges'<br>" +
		...
		edgesFuzzyRadioButton.setToolTipText(
				"<html>Activates the 'Fuzzy edges'<br>" +
		...
		edgesFuzzyRatioSlider.setToolTipText(
				"<html>Controls the utility ratio used<br>" +
		...
		edgesFuzzyPercentageSlider.setToolTipText(
				"<html>Determines the minimal utility for<br>" +
		...
		edgesFuzzyIgnoreLoopBox.setToolTipText(
				"<html>If active, length-1-loops (i.e.,<br>" +
		...
		edgesFuzzyInterpretAbsoluteBox.setToolTipText(
				"<html>If active, all edges' utility value<br>" +
		...
		edgesConcurrencyThresholdSlider.setToolTipText(
				"<html>For conflicting relations, this sets the<br>" +
		...
		concurrencyPreservePanel.add(packVerticallyCentered(edgesConcurrencyThresholdLabel, 40, 20), BorderLayout.SOUTH);
		...
		edgesConcurrencyRatioSlider.setToolTipText(
				"<html>For conflicting relations which have fallen<br>" +
		...
		edgesConcurrencyActiveBox.setToolTipText(
				"<html>This control can be used to switch off<br>" +
	...
	public ProvidedObject[] getProvidedObjects() {
		ProvidedObject fuzzyGraphObject =
			new ProvidedObject("Fuzzy Graph",
				   new Object[] {
					(FuzzyGraph)graph
		});
		if(log != null) {
			...
			try {
				...
				FuzzyGraphProjectionFilter projectionFilter = new FuzzyGraphProjectionFilter(graph);
				projectionFilter.setLowLevelFilter(log.getLogFilter());
				filteredLog = LogReaderFactory.createInstance(projectionFilter, log);
			} catch (Exception e) {
				e.printStackTrace();
				return null;
			}
			ProvidedObject filteredLogObject =
				new ProvidedObject("Filtered log (projected on fuzzy graph)",
					   new Object[] {
						filteredLog
			});
				...
				graphPanel.setGraph(new GGAdapter(graph), new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						if(log != null) {
							...
							Message.add("Starting fuzzy graph replay...", Message.NORMAL);
							...
							try {
								...
								Message.add("...replayed fuzzy graph in " + ((System.currentTimeMillis() - start) / 1000) + " seconds.", Message.NORMAL);
								Message.add("Measured fuzzy conformance is " + replay.getValue(), Message.NORMAL);
								replayGauge.setCoverage(replay.getValue());
							} catch (IOException ioe) {
								// no way to fix this here...
								Message.add("Fatal error in Fuzzy Miner: please check your STDERR output for stack trace!", Message.ERROR);
								ioe.printStackTrace();
							} finally {
								setGuiEnabled(true);
							}
	...
	protected double getConcurrencyThresholdFromSlider() {
		double value = Math.pow(((double)edgesConcurrencyThresholdSlider.getValue() / 1000.0), 4.0);
		return value;
	}
