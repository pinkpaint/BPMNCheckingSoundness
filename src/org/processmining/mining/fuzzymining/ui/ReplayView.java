...
public class ReplayView extends JPanel {
	...
	public ReplayView(FuzzyReplay replay) {
		this.replay = replay;
		...
		for(int i=0; i<replay.numberOfTraces(); i++) {
			TraceReplayView traceView = new TraceReplayView(replay.getTraceReplay(i));
			traceView.setAlignmentX(LEFT_ALIGNMENT);
		...
		headerBar = new HeaderBar("trace conformance inspector");
