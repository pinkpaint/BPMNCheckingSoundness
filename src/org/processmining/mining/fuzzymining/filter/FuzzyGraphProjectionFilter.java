	...
	protected boolean doFiltering(ProcessInstance instance) {
		...
		for(int i=0; i<ateList.size(); i++) {
			try {
				...
				int logEventIndex = events.findLogEventNumber(current.getElement(), current.getType());
				if(logEventIndex < 0) {
					...
					if(node == null) {
						// not found
						Message.add("Fuzzy Graph projection filter: Could not find log event for " 
								+ current.getElement() + "/" +  current.getType() + "!", Message.WARNING);
					}
				} else {
				...
				if(node == null) {
					...
					if(lastCluster != null && lastCluster.equals(cluster)) {
						...
						current.setAttribute("FMFILTER_OriginalElement", current.getElement());
						current.setAttribute("FMFILTER_OriginalEventType", current.getType());
