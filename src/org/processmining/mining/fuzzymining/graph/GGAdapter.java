...
public class GGAdapter extends GGGraph {
	...
	public GGAdapter(FuzzyGraph fuzzyGraph) {
		...
		primitiveNodePainter = 
			new GGNodeRoundRectanglePainter(PRIMITIVE_BACKGROUND, PRIMITIVE_BORDER, PRIMITIVE_TEXT);
		clusterNodePainter = 
			new GGNodeOctagonPainter(CLUSTER_BACKGROUND, CLUSTER_BORDER, CLUSTER_TEXT);
	...
	public static String[] getNodeLabel(Node node) {
		if(node instanceof ClusterNode) {
			ClusterNode cluster = (ClusterNode)node;
			return new String[] {cluster.getElementName(), cluster.size() + " elements", 
	...
	protected class GGFuzzyGraphAdapter extends GGGraphAdapter {
		...
		/* (non-Javadoc)
		 * @see org.processmining.mining.fuzzymining.gantzgraf.util.GGGraphAdapter#nodeSelected(org.processmining.mining.fuzzymining.gantzgraf.painter.GGCanvas, org.processmining.mining.fuzzymining.gantzgraf.model.GGGraph, org.processmining.mining.fuzzymining.gantzgraf.model.GGNode, float, float)
		 */
		@Override
		public void nodeSelected(GGCanvas canvs, GGGraph graph, GGNode node,
			...
			try {
				MainUI.getInstance().createFrame("Detailled graph for 'Cluster " 
						+ cluster.getIndex() + "'", cluster.getClusterGraphPanel());
			} catch (Exception e) {
				// oops... SNAFU
				e.printStackTrace();
			}
