	...
	protected void processEdgesForNode(int index) {
		...
		for(int i=0; i<numberOfInitialNodes; i++) {
			...
			if(sig > 0.0) {	// valid relation
				...
				if(inValues[i] > maxInVal) { maxInVal = inValues[i]; }
				if(inValues[i] < minInVal) { minInVal = inValues[i]; }
			} else {
				inValues[i] = 0.0;
			}
			...
			if(sig > 0.0) {	// valid relation
				...
				if(outValues[i] > maxOutVal) { maxOutVal = outValues[i]; }
				if(outValues[i] < minOutVal) { minOutVal = outValues[i]; }
			}  else {
				outValues[i] = 0.0;
			}
		}
