...
public class FastTransformer extends FuzzyGraphTransformer {
	...
	public void transform(MutableFuzzyGraph graph) {
		...
		for(FuzzyGraphTransformer pre : preTransformers) {
			Message.add("  * Applying pre-flight graph transformer '" + pre.getName() + "'.", Message.NORMAL);
			pre.transform(graph);
		}
		...
		for(FuzzyGraphTransformer interim : interimTransformers) {
			Message.add("  * Applying in-flight graph transformer '" + interim.getName() + "'.", Message.NORMAL);
		...
		for(FuzzyGraphTransformer post : postTransformers) {
			Message.add("  * Applying post-flight graph transformer '" + post.getName() + "'.", Message.NORMAL);
	...
	protected ArrayList<ClusterNode> removeIsolatedClusters(ArrayList<ClusterNode> clusters) {
		Set<Node> preset;
		Set<Node> postset;
		int stopCounter = clusters.size();
		ClusterNode cluster;
	...
	protected ArrayList<ClusterNode> removeSingularClusters(ArrayList<ClusterNode> clusters) {
		int stopCounter = clusters.size();
		ClusterNode cluster;
		for(int index = 0; index < stopCounter;) {
			cluster = clusters.get(index);
	...
	protected void eliminateSingularClusterPreservingLinks(ClusterNode cluster) {
		...
		for(Node pre : preSet) {
			if(pre instanceof ClusterNode) { continue; }
			preIndex = pre.getIndex();
			for(Node post : postSet) {
				if(post instanceof ClusterNode) { continue; }
	...
	protected Node getMostSignificantNeighbor(Node node) {
		...
		Node winner = null;
		for(int i=graph.getNumberOfInitialNodes()-1; i>=0; i--) {
			if(i == refIndex) { continue; }	// skip self
