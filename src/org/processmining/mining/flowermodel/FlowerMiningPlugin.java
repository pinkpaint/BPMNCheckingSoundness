...
/**
 * Simple mining plugin that constructs a so-called "Flower model" from the given event log.
...
public class FlowerMiningPlugin implements MiningPlugin {
	...
	public MiningResult mine(LogReader log) {
		...
		Iterator<LogEvent> allEvents = log.getLogSummary().getLogEvents().iterator();
		LogEvent current;
	...
	public class FlowerResult implements MiningResult, Provider {
		...
		public ProvidedObject[] getProvidedObjects() {	
			return new ProvidedObject[] {
					new ProvidedObject("Flower Model", new Object[] {model, logReader})
			};
		}
	}
