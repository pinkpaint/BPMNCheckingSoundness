...
public class TraceClusterSet implements DotFileWriter {
	...
	public void cluster(double maxDiameter) {
		...
		while(traces.size() > 0) {
			Message.add("Building next generation from " + traces.size() + " traces...", Message.NORMAL);
	...
	public TraceCluster deriveNextCluster(List<SingleTraceStat> traces, double maxDiameter) {
		ArrayList<TraceCluster> nextClusters = new ArrayList<TraceCluster>(traces.size());
	...
	public TraceCluster buildCluster(SingleTraceStat initialTrace, List<SingleTraceStat> traces, double maxDiameter) {
		...
		public double getMaxDiameter() {
			...
			for(SingleTraceStat source : traces) {
				for(SingleTraceStat target : traces) {
					distance = comparator.getRelation(source.getTraceIndex(), target.getTraceIndex());
