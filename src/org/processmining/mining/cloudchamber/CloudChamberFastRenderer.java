...
public class CloudChamberFastRenderer {
	...
	private static int[][] COLOR_ENCODED = {
		{
			encodeColor(COLOR_CHART[0][0]),
			encodeColor(COLOR_CHART[0][1]),
			encodeColor(COLOR_CHART[0][2]),
			encodeColor(COLOR_CHART[0][3]),
			encodeColor(COLOR_CHART[0][4])
		},
		{
			encodeColor(COLOR_CHART[1][0]),
			encodeColor(COLOR_CHART[1][1]),
	...
	public CloudChamberFastRenderer(CloudChamberStats ccStats, JComponent aComponent) {
		...
		if(width > imgWidth || height > imgHeight || x != oldX || y != oldY) {
			...
			for(int aY=0; aY<height; aY++) {
				for(int aX=0; aX<width; aX++) {
					...
					//pixels[index] = encodeColor(stats.getValueAt(x + aX, y + aY));
	...
	public static BufferedImage getGradient(int width, int height) {
		...
		if(width > height) {
			for(int x=0; x<width; x++) {
				//g2d.setColor(new Color(encodeColor((double)x / (double)(width - 1))));
				g2d.setColor(new Color(mapColor((double)x / (double)(width - 1))));
				g2d.drawLine(x, 0, x, height - 1);
			}
		} else {
			for(int y=0; y<height; y++) {
				//g2d.setColor(new Color(encodeColor(1.0 - ((double)y / (double)(height - 1)))));
				g2d.setColor(new Color(mapColor(1.0 - ((double)y / (double)(height - 1)))));
