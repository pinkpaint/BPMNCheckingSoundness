...
public class HeuristicsNet2FuzzyModel implements ConvertingPlugin {
	...
	protected void addSuccessorEdgesForIndex(HeuristicsNet net, FuzzyModelCreator kreator, int index) {
		...
		for(int i=0; i<net.size(); i++) {
			if(outSetElements.contains(i)) {
				LogEvent successorEvent = net.getLogEvents().get(net.getDuplicatesMapping()[i]);
