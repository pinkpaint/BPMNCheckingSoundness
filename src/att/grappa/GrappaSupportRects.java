...
public class GrappaSupportRects implements GrappaConstants {
	...
	protected static synchronized Object[] parseRecordInfo(Node node) {
		...
		double height = ((Double) node.getAttributeValue(HEIGHT_ATTR)).doubleValue() *
						PointsPerInch;
		...
		tableField.positionFields(new Point((int) Math.round(pos.getX() - width / 2.0),
				(int) Math.round(pos.getY() - height / 2.0)));
	...
	private static TableField doParse(Node node, boolean LR, boolean topLevel) {
		...
		for (int pos = arrayOffset; pos < parseArray.length; pos++) {
			if (parseArray[pos] == '\\') {
				pos++;
				if (pos < parseArray.length && (parseArray[pos] == '{' || parseArray[pos] == '}' ||
						parseArray[pos] == '|')) {
					continue;
				}
			}
...
class TableField implements GrappaConstants {
	...
	void setTextBounds(String str, Node node) {
		...
		for (int i = 0; i < array.length; i++) {
			...
			value = array[i] - 32;
			fwidth += (cwFont) ? fontwidth[0] : ((value >= 0 && value < fontwidth.length) ?
					fontwidth[value] : 0);
		}
		if (fwidth > xwidth) {
			xwidth = fwidth;
