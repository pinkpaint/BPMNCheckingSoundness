...
public final class Attribute extends java.util.Observable implements
		...
		switch (attrtype) {
			...
			default:
				if (Attribute.specialHandler != null) {
					...
					throw new RuntimeException(Element.typeString(type,
							true) + " attribute (" + name + ") needs a special handler");
				}
		}
		...
		if (stringValue == null && value != null) {
			if (value instanceof String) {
				...
				throw new RuntimeException(
						"AttributeHandler needed to perform conversion of attribute \"" + name +
	...
	public Object convertStringValue(int type, String name, String stringValue, int attrtype) {
		...
		if (stringValue == null ||
				(stringValue != null && attrtype != STRING_TYPE && stringValue.trim().length() == 0)) {
			value = null;
			stringValue = null;
		} else {
			...
			switch (attrtype) {
				...
				case DOUBLE_TYPE:
					try {
						...
						throw new IllegalArgumentException("bad number format (" + stringValue +
								") for attribute \"" + name + "\"");
					}
					break
							;
				...
				case INTEGER_TYPE:
					try {
						...
						throw new IllegalArgumentException("bad integer format (" + stringValue +
								") for attribute \"" + name + "\"");
					}
					break
							;
				...
				case SHAPE_TYPE:
					if ((value = (Integer) Grappa.keyToShape.get(stringValue)) == null) {
						Attribute attr;
						if ((attr = Graph.getGlobalAttribute(Grappa.NODE,
									"shape")) == null ||
									(value = (Integer) Grappa.keyToShape.get(attr.getStringValue())) == null) {
							throw new InternalError("could not provide default when unkown shape (" +
									stringValue + ") supplied for attribute \"" + name + "\"");
						}
					}
					break;
				case SIZE_TYPE:
				...
				default:
					if (Attribute.specialHandler != null) {
						value = Attribute.specialHandler.convertStringValue(type, name, stringValue,
								attrtype);
					} else {
						throw new RuntimeException(Element.typeString(type,
								true) + " attribute (" + name + ") needs a special handler");
	...
	public Object copyValue(int type, String name, Object value, int attrtype) {
		...
		if (copy_value == null && value != null) {
			if (value instanceof String) {
				...
				throw new RuntimeException(
						"AttributeHandler needed to perform copy of attribute \"" + name +
						"\", please supply one via Attribute.setAttributeHandler()");
			}
		}
